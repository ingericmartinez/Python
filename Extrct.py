...
class JavaClassExporter:
    ...
    def scan_java_files(self):
        ...
        try:
            ...
            # ORDENAR LAS CLASES ALFABÉTICAMENTE
            self.java_files.sort(key=lambda x: x[1].lower())  # Ordenar por nombre (case insensitive)
            
            # Insertar en el treeview ordenado
            for full_path, name, file_type in self.java_files:
                self.tree.insert("", "end", values=("◻", name, file_type, full_path))
    ...
    def export_java_file(self, file_path, output_dir, exported_files, depth=0):
        ...
        try:
            ...
            # MANTENER EL PACKAGE ORIGINAL - NO MODIFICAR
            # Solo copiar el contenido original sin cambios
            new_content = content
